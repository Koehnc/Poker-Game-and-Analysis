...
- **No visual marker for a player's current bet.** When someone bets/calls/raises, the only feedback is the history log text and the updated pot total/chip stack — there's nothing sitting on the table near their seat showing what they've put in this street, the way a chip stack does at a real table. `HandStep`/`GameStateView` don't currently expose a per-player current-street bet amount to the UI at all (the engine tracks it internally as `Poker::mBetsFromPlayers`, reset each street in `betsIn`'s cleanup, but it's private and never surfaced). Would need that value added to the step data, plus a small label/marker per seat in `GameWidget` that updates on `PlayerActed` and clears on `StreetDealt`/`HandStarted`.
